...
- Uses the same 5% eval split as training (seed=42).
- Reports **BLEU** and prints the first 5 Marathi → reference vs prediction.

## Translate CSV → CSV

Use the fine-tuned model to translate a CSV file (one Marathi column → one English column):

```bash
python translate_csv.py input.csv output.csv
```

- **Input:** CSV with a column containing Marathi text (column auto-detected if named `marathi`, `marathi_text`, `source`, etc., or use first column).
- **Output:** Same CSV with an extra column `english_text` (or `--output-column` / `OUTPUT_COLUMN`).

**Options:**

| Option / Env              | Description |
|---------------------------|-------------|
| `--marathi-column` / `MARATHI_COLUMN` | Column name for Marathi text |
| `--output-column` / `OUTPUT_COLUMN`   | Output column name (default: `english_text`) |
| `--batch-size` / `TRANSLATE_BATCH_SIZE`| Inference batch size (default: 16) |

Example with custom column names:

```bash
python translate_csv.py data.csv translated.csv --marathi-column "Marathi" --output-column "English"
```
